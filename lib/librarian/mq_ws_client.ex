defmodule Librarian.MqWsClient do
  @moduledoc """
  ...
  This complements the existing `Librarian.IAMQ` GenServer which
  ...
  defp handle_incoming_message(message, _state) do
    ...
    # Log message to data folder (reuses Librarian.IAMQ's logging convention)
    log_to_data_folder(message)
  ...
  defp route_request(message) do
    ...
    cond do
      String.contains?(subject, "search") or String.contains?(subject, "find") ->
        Logger.info("[MQ-WS] Search request from #{from}")

      String.contains?(subject, "archive") or String.contains?(subject, "store") ->
        Logger.info("[MQ-WS] Archive request from #{from}")

      String.contains?(subject, "summarize") or String.contains?(subject, "summary") ->
        Logger.info("[MQ-WS] Summarize request from #{from}")
  ...
  defp log_to_data_folder(message) do
    ...
    if log_dir do
      ...
      log_file = Path.join(log_dir, "iamq-ws-#{safe_ts}-#{from}.json")
      File.write!(log_file, Jason.encode!(message, pretty: true))
    end
  rescue
    e -> Logger.warning("[MQ-WS] Failed to log message: #{Exception.message(e)}")
  end
end
